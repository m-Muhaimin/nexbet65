import React from 'react';
import { motion } from 'motion/react';
import { X, Settings, Volume2, VolumeX, Zap } from 'lucide-react';

interface SettingsModalProps {
  isOpen: boolean;
  isMuted: boolean;
  isTurbo: boolean;
  onToggleMute: () => void;
  onToggleTurbo: () => void;
  onClose: () => void;
}

export const SettingsModal: React.FC<SettingsModalProps> = ({
  isOpen,
  isMuted,
  isTurbo,
  onToggleMute,
  onToggleTurbo,
  onClose,
}) => {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/80 z-[70] flex items-center justify-center p-3 backdrop-blur-sm select-none">
      <motion.div
        initial={{ scale: 0.9, opacity: 0 }}
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.9, opacity: 0 }}
        className="w-full max-w-sm bg-gradient-to-b from-[#132036] to-[#0a1424] border-2 border-[#b98a2e] rounded-2xl p-4 flex flex-col shadow-[0_12px_36px_rgba(0,0,0,0.95)]"
      >
        {/* Header */}
        <div className="w-full flex items-center justify-between pb-2 border-b border-[#22304a]">
          <div className="flex items-center gap-1.5 text-[#f6b01a]">
            <Settings className="w-4 h-4" />
            <span className="font-['MStiffHei_PRC_UltraBold'] font-extrabold text-sm text-[#fff6d8] uppercase">
              Game Settings
            </span>
          </div>
          <button
            onClick={onClose}
            aria-label="Close"
            className="w-7 h-7 rounded-full btn-dark flex items-center justify-center text-[#cfd6e4] hover:text-white cursor-pointer active:scale-95"
          >
            <X className="w-4 h-4" />
          </button>
        </div>

        {/* Options List */}
        <div className="flex flex-col gap-2.5 my-4">
          {/* Sound Toggle */}
          <div className="p-3 rounded-xl bg-[#0d1728] border border-[#1e2e48] flex items-center justify-between">
            <div className="flex items-center gap-3">
              {isMuted ? (
                <VolumeX className="w-5 h-5 text-red-400" />
              ) : (
                <Volume2 className="w-5 h-5 text-[#2ecc71]" />
              )}
              <div>
                <div className="text-xs font-bold text-white">Sound Effects</div>
                <div className="text-[10px] text-[#aab]">Web Audio synthesized beeps</div>
              </div>
            </div>

            <button
              onClick={onToggleMute}
              className={`w-12 h-6 rounded-full transition-colors relative cursor-pointer ${
                !isMuted ? 'bg-[#f6b01a]' : 'bg-[#1e2e48]'
              }`}
            >
              <div
                className={`w-5 h-5 rounded-full bg-white transition-transform absolute top-0.5 ${
                  !isMuted ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>

          {/* Turbo Mode Toggle */}
          <div className="p-3 rounded-xl bg-[#0d1728] border border-[#1e2e48] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Zap
                className={`w-5 h-5 ${
                  isTurbo ? 'fill-[#f6b01a] text-[#f6b01a]' : 'text-[#aab]'
                }`}
              />
              <div>
                <div className="text-xs font-bold text-white">Turbo Mode</div>
                <div className="text-[10px] text-[#aab]">Fast spins and quick cascades</div>
              </div>
            </div>

            <button
              onClick={onToggleTurbo}
              className={`w-12 h-6 rounded-full transition-colors relative cursor-pointer ${
                isTurbo ? 'bg-[#f6b01a]' : 'bg-[#1e2e48]'
              }`}
            >
              <div
                className={`w-5 h-5 rounded-full bg-white transition-transform absolute top-0.5 ${
                  isTurbo ? 'translate-x-6' : 'translate-x-1'
                }`}
              />
            </button>
          </div>

        </div>

        {/* Close button */}
        <button
          onClick={onClose}
          className="w-full py-2 rounded-xl btn-dark text-xs font-bold text-[#aab] uppercase hover:text-white cursor-pointer active:scale-95"
        >
          Close
        </button>
      </motion.div>
    </div>
  );
};
