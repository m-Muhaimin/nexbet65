...
3. **Texture utilities**:
   ...
   - `.hero-radial` — `radial-gradient(circle at top right, rgba(163,230,53,.12), transparent 60%)` (lobby multiplier panel, coming-soon hero).
...
- **Category pills** (`category-filters.tsx`): `.glass sticky top-14 z-30 -mx-3 border-b border-white/5`, horizontal scroll (`no-scrollbar`). Pills: `rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-sm text-white/70`; active = `.pill-active` (`bg #f6b01a`, black text, 600). Keys: all / crash / live / table / hot / new. Sidebar + pill bar share state via `lobby-bus`.
